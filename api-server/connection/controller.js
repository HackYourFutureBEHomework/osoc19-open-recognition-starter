...
controller.welcome = (req, res) => {
  console.log("-- GET /all --");
  res.send("hi, you're at the connection api");
}

/* ------------------- INDEX -------------------*/
// GET  "/all"                     => show all connection
...
/* ------------------- READ -------------------*/
// GET  "/find/:id"             => View connection Info with id ...
...
controller.post_id_update = (req, res) => {
    ...
    connection.update(id_object, title_object)
      // modify the next line based on your project's needs        
      .then((db_response) => { res.send(db_response) })
      .catch((err) => {
        res.status(500).send({
