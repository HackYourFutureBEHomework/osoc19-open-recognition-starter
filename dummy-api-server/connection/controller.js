...
controller.get_add = (req, res) => {
    console.log("-- GET /add --");
    res.send('put a new connection\'s name in the body and send a post request to this address: { first_name: "string" }. you\'ll get back the new connection\'s entry');
}

controller.post_add = (req, res) => {
    console.log("-- POST /add --");
    const new_name = req.body.first_name;
    const new_profile = { first_name: new_name };
    authors.create(new_profile)
      // modify the next line based on your project's needs
      .then((db_response) => { res.send(db_response) })
      .catch((err) => {
        res.status(500).send({
